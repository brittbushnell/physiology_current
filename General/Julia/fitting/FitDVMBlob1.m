function [param, bhat, funbest] = FitDVMBlob1(x, R, nStarts, param0)
% VER. 1 %%%%%%%%
% 2 vonMises*Gaussian blobs. Same standard deviation terms in both
% dimensions but allowed to vary in amplitude.
%
% Inputs:
...
fnBounds = @(sc1, sc2, mux, muy, kappa, sdy) double(...
    (mux < 0) | (mux > max(x)) | (muy < 0) | (muy > max(y)) | ...
    (kappa <= 0) | (kappa > 20) | (sdy <= 0) ) * 1/eps;

% Minimize the squared error.
fnOptim = @(beta) sum(sum(( fnDVMBlob(beta(1),beta(2),beta(3),beta(4),beta(5),beta(6)) - mm).^2 ))+ ...
    fnBounds(beta(1),beta(2),beta(3),beta(4),beta(5),beta(6));
...
bhat = fnDVMBlob(param(1), param(2), param(3), param(4), param(5), param(6));
