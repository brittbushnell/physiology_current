function R = structBlackrock_gratings(S, filename, outputDir, info, binStart, binEnd, forceOverwrite)
%
...
if ~exist(saveFile) || forceOverwrite
    ...
    nStimShown = length(S.t_stim);
    
    S.rotation  = mod(S.rotation, 180);
    oris        = unique(S.rotation);                                             % Effectively 6 oris and 6 sfs.
    sfs         = unique(S.spatial_frequency(S.spatial_frequency > 0.0006));       % 0 is blank. 0.003 and 0.006 are black/white flashes
    nOri        = length(oris);
    nSf         = length(sfs);
    blankind    = find(S.spatial_frequency == 0);
...
%         suptitle('all trials per channel - outliers removed')
    ...
    [temp, ~]       = hist(S.rotation.*S.spatial_frequency, unique(S.rotation.*S.spatial_frequency));
    ...
    for iC_ = 1:nChan
        iC          = aMap.channel(iC_);
        blanks      = S.bins(blankind,1:nBins,iC);
        ...
        for iOri = 1:nOri
            for iSf = 1:nSf
                ind                     = intersect(find(S.rotation == oris(iOri)), find(S.spatial_frequency == sfs(iSf)));
