...
%   E-mail:      romesh.kumbhani@nyu.edu
    ...
    if size(expoDataSet.waveforms, 1) == 0
        error('ExpoMatlab:GetWaveforms','There is no waveform data.')
    end
    ...
    if isfield(expoDataSet.waveforms,'DataSize')
        dataSize = expoDataSet.waveforms.DataSize;
    else
        dataSize = 1;
    end
    ...
    if exist('waveformUnit','var') && ~isempty(waveformUnit) 
    ...
    if ~isnumeric(passIDs)
        error('ExpoMatlab:GetWaveforms','passIDs should be a vector of numbers. It appears to be non numeric.');
    end

    if ~isnumeric(channelID)
        error('ExpoMatlab:GetWaveforms','channelID is non numeric.');
    elseif size(channelID, 2) ~= 1
        error('ExpoMatlab:GetWaveforms','channelID should be a scalar value.');
    end

    if channelID + 1 > numOfChannels 
        error('ExpoMatlab:GetWaveforms','channelID %d is out of range. There are only %d channels starting at channel 0.', channelID, numOfChannels);
    elseif channelID<0
        error('ExpoMatlab:GetWaveforms','%d is an invalid channelID value. It must be 0 or more.', channelID);
    end
    
    if isDuration~=0 && isDuration ~=1
        error('ExpoMatlab:GetWaveforms','isDuration should be either 0 or 1.');
    elseif isDuration == 1 && max(endTimes) <= 0
        error('ExpoMatlab:GetWaveforms','isDuration is set to 1 but the durations are all negative or 0.');
    end
    ...
    if numOfStartTimes > 1  && numOfPasses ~= numOfStartTimes
        error('ExpoMatlab:GetWaveforms','startTimes must be either a scalar or a vector of same length as passIDs.  startTimes contains %d values whereas passIDs contains %d.', size(startTimes), numOfPasses);
    end

    if numOfEndTimes > 1  && numOfPasses ~= numOfEndTimes
        error('ExpoMatlab:GetWaveforms','endTimes must be either a scalar or a vector of same length as passIDs.  endTimes contains %d values whereas passIDs contains %d.', size(endTimes), numOfPasses);
    end

    if max(expoDataSet.passes.IDs) < max(passIDs)
       error('ExpoMatlab:GetWaveforms','There appear to be out of range values in the passIDs vector becuase the maximum value within passIDs %d exceeds the maximum value within expoDataSet %d. ', max(passIDs), max(expoDataSet.passes(:, 1)));
    end
    ...
    firstSampleTimes = nan(1,numOfPasses);
    ...
    for i = 1:numOfPasses
        ...
        if size(passNum, 2) < 1
            warning('ExpoMatlab:GetWaveforms','No entry for passID %d found in expoDataSet', i);
            continue
        end
        ...
        if packetNum == 0
            ...
            packetNum = find(expoDataSet.waveforms.Times >= adjustedPassStartTime & expoDataSet.waveforms.Times < adjustedPassEndTime, 1);
        ...
        if packetNum > 0
            ...
            while 1
                ...
                startSamplePos =  dataSize * firstSampleNum * numOfChannels + channelID + 1;
                ...
                endSamplePos = dataSize * (lastSampleNum - 1) * numOfChannels + channelID + 1;
                ...
                data = double(packetData(startSamplePos:dataSize*numOfChannels:endSamplePos));
                if dataSize == 2 % 16 bit
                    data = data  + 256*double(packetData(startSamplePos+1:dataSize*numOfChannels:endSamplePos+1));
                end
                ...
                %[endTimeOfPacket startTimeOfNextPacket startTimeOfNextPacket-endTimeOfPacket sampleInterval]
