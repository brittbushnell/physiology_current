...
if exist('matrixElement')
    ...
    if numOfDimensions ~= 0
        % calculate the blockName pointed to by the matrixElement
        blockName = '';
        
	for i=1:numOfDimensions 
		value = matrixElement(i);
		dimensionSize = expoDataSet.matrix.Dimensions{i}.Size;
        	dimensionName = expoDataSet.matrix.Dimensions{i}.Name;
            
            	blockName = sprintf('%s[%s:%d]', blockName, dimensionName, value);
        ...
        % got blockName now get blockNum
        blockNum = strmatch(blockName, expoDataSet.blocks.Names, 'exact');
        
        blockID = expoDataSet.blocks.IDs(blockNum);
