function expoDataSet = ReadExpoXML(filename, doSpikeWaveforms, beVerbose, doFrames)
% function ReadExpoXML(filename, doSpikeWaveforms, beVerbose, doFrames)
%
...
% doFrames: 1 if you want to process frame timestamps, 0 to skip. [default 1]
%
...
%
%   Author:        Romesh Kumbhani
%	Version:       2.2 (made frameticks optional)
...
doframes = 0;
if exist('doFrames','var') && doFrames ~= 0
    doframes = 1;
end
...
if isfield(output,'frames')&&isfield(output.frames,'ExpoStartTime_ns')&&~isnan(output.frames.ExpoStartTime_ns)
...
%  output.ExpoVersion         : The version of Expo used to generate the XML
%  output.MatlabImportVersion : This version of ReadExpoXML

% if expo version is newer than what's supported in this file
%if ExpandVersion(output.ExpoVersion) > ExpandVersion(output.MatlabImportVersion)
%    warning('ExpoMatlab:ReadExpoXML','\nThis XML file was generated using an Expo v%s. This ReadExpoXML will not support features newer than v%s. Please use an updated ExpoMatlab library otherwise functions may break.\n',output.ExpoVersion,output.MatlabImportVersion);
%end

if verbose
    fprintf('\nExpoVersion: %s\nMatlabImportVersion: %s\nFile name: %s\n', output.ExpoVersion,output.MatlabImportVersion,output.FileName);
end
return
...
if ~isempty(fields)
    ...
    frames.ExpoStartTime_ns = NaN;
    frames.ExpoEndTime_ns   = NaN;
end

if strcmp(currentLine(end-1),'/')
    return;
end
...
function ParseXML(doWaveforms,doframes)
global textfile tokenlist currentLine linenum ticknum;
