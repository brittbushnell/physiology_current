function S = oepExpoRvcOrt(azDir,unitLabel,runNum,doBoot,doPlot)
...
azFile = [azDir,'/',runStr,'_rvcOrtAz.mat'];

if ~exist(azFile,'file') || forceOverwrite
    fprintf(['oepExpoRvcOrt: ',runStr,'\n'])
    ...
    maskOD = [nan;GetEvents(ExpoIn,passIDs,'Surface','',1,'Width')];
    maskODvals = unique(maskOD(2:end));
    isMaskODreg = maskOD == median(maskODvals); % Middle OD size if 3 were used. Fix later to use other sizes
    isMask = con2>0 & isMaskODreg;
    for iM = 1:2
        for iC = 1:nCon
            if iM==1
