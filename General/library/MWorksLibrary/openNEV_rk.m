function varargout = openNEV_rk(varargin)
...
Trackers.fBasicHeader = ftell(FID);
...
% fprintf('%d ExtendedHeaders\n',Trackers.countExtHeader);
% aaa = 0;
% eee = 0;
% ccc = 0;
% mmm = 0;
...
if NEV.MetaTags.PacketCount ~= 0
    ...
    % read it all one, but only once! - rk
%     tic;
%     fseek(FID, Trackers.fExtendedHeader, 'bof');
%     tRawData  = fread(FID, [Trackers.countPacketBytes Trackers.countDataPacket], '*uint8'); % rk
%     toc;
    tic;
    fseek(FID, Trackers.fExtendedHeader, 'bof');
    totalbytes = Trackers.countPacketBytes*Trackers.countDataPacket;
    tb64 = floor(totalbytes/8);
    tb64_remain = mod(totalbytes,8);
    tmp = fread(FID,tb64,'*uint64');
    tmp = typecast(tmp,'uint8');
    if (tb64_remain>0)
        tmp2=fread(fid,tb64_remain,'*uint8');
        tRawData = reshape([tmp;tmp2],Trackers.countPacketBytes,Trackers.countDataPacket);
    else
        tRawData = reshape(tmp,Trackers.countPacketBytes,Trackers.countDataPacket);
    end
    ...
    if ~exist('readTime', 'var')
        ...
        [~,tempReadPackets] = find(Timestamp > readTime(1)*NEV.MetaTags.SampleRes,1,'first');
        ...
        [~,tempReadPackets] = find(Timestamp < readTime(2)*NEV.MetaTags.SampleRes,1,'last');
        if ~isempty(tempReadPackets)
            ...
            % rk
            fprintf('The file contains %0.2f seconds of data. The requested ending timestamp of %0.2f seconds is earlier than the first timestamp.\n', ...
                double(Timestamp(end))/double(NEV.MetaTags.SampleRes), ...
                readTime(2));
            clear variables;
            if nargout
        ...
        clear tempReadPackets;
        
        % grab subset of Timestamp and rawdata specific to the region
        % requested.
        Timestamp = Timestamp(:,Trackers.readPackets(1):Trackers.readPackets(2));
        tRawData  =  tRawData(:,Trackers.readPackets(1):Trackers.readPackets(2));
    end
    ...
    tempClassOrReason = uint8(tRawData(7,:)); % rk
...
digserIndices              = (PacketIDs == digserPacketID);
neuralIndices              = (neuralIndicesPacketIDBounds(1) <= PacketIDs & PacketIDs <= neuralIndicesPacketIDBounds(2));
commentIndices             = (PacketIDs == commentPacketID);
videoSyncPacketIDIndices   = (PacketIDs == videoSyncPacketID);
trackingPacketIDIndices    = (PacketIDs == trackingPacketID);
patientTrigPacketIDIndices = (PacketIDs == patientTrigPacketID);
reconfigPacketIDIndices    = (PacketIDs == reconfigPacketID);
...
NEV.Data.Spikes.FileOffset = ((find(neuralIndices)-1)*NEV.MetaTags.PacketBytes) + NEV.MetaTags.HeaderOffset;
clear PacketIDs;
...
clear tempClassOrReason tempDigiVals;

%% Reads all the waveforms...
% allExtraDataPacketIndices  = [...
%     commentIndices, ...
...
if any(allExtraDataPacketIndices) % if there is any extra packets    
    if any(commentIndices)
        ...
        neuroMotiveEvents = (NEV.Data.Comments.CharSet == 255);
    ...
    if any(videoSyncPacketIDIndices)
    ...
    if any(trackingPacketIDIndices)
    ...
    if any(patientTrigPacketIDIndices)
...
clear Timestamp count idx;

if strcmpi(Flags.ReadData, 'read')
    % now read waveform    
    NEV.Data.Spikes.WaveformUnit = Flags.waveformUnits;
    
    tmp = tRawData(9:Trackers.countPacketBytes,neuralIndices);    
    NEV.Data.Spikes.Waveform = reshape(typecast(tmp(:),'int16'),(Trackers.countPacketBytes-8)/2,sum(neuralIndices));
    %NEV.Data.Spikes.Waveform(:, [digserIndices allExtraDataPacketIndices]) = [];
    
    clear allExtraDataPacketIndices neuralIndices;
    ...
    if strcmpi(Flags.ParseData, 'parse')
        try
            ...
            for idx = 1:length(splitDigiValues)
                try
                    if isempty(find(splitDigiValues{idx} == ':', 1))
                        splitDigiValues{idx}(splitDigiValues{idx} == '#') = [];
            ...
            if ~isempty(DataBegTimestamp)
                ...
                c = Inputs(NEV.Data.SerialDigitalIO.InsertionReason(AsteriskIndices));
                [NEV.Data.SerialDigitalIO.InputType] = deal(c{1:end});
            end
