%CUMGAUSSFIT fits a cummulative gaussian
%
%   This function fits a Gaussian or normal c.d. function through the data.
%
%   This function is partly a wrapper for ANYFIT.
%
%   The function is defined as (see [1]):
%       f(x|bias,gain,m,s) = bias + gain * \frac{1}{s\sqrt{2\pi}}\int_{-\infty}^{x}e^{-\frac{(x-m)^2}{2s^2}}dt 
%           x: the independent variable (e.g. stimulus)
%           f(x): dependent variable (e.g. response)
%       constants or parameters:
%           lapseLo: lower lapse rate
%           lapseUp: upper lapse rate
%           m: mu, mean, peak
%           s: sigma, shape, variance
%       Note that amplitude is maximal 1 when lapse rates are zero
%
% [1] https://en.wikipedia.org/wiki/Normal_distribution#Cumulative_distribution_function
%
%   Input:
%   cumgaussfit(x,y)
%       x :	independent variable 
%       y :	dependent variable
%       Optional arguments: These come in pairs
%           'upperbound' , 'ub'
%           'lowerbound' , 'lb': fit restrictions
%               Default [0 0 -inf 0] and [+inf +inf +inf +inf]
%           'initialguess', 'ig': initial guess
%               Default uses initialguess_.. routine
%           'silent': Do not output to screen.
%               Default false
%
%   Output:
%   [beta, betavar, function, stat, betacon] = cumgaussfit(..)
%       beta :      fitted parameters
%       betavar :   variance of betas
%       function :  Function handle
%       stat :      Statistical information (see anyfit)
%       betacon :   As beta, but with constrained (fixed) betas excluded.
%
% v1 TvG Oct 2015, NYU: added help. outsourced to anyfit

function [varargout]=cumgaussfit(Xdata,Ydata,varargin)

%%
cumgaussfun = @(b,x) b(1) + ((1-b(1)-b(2))*normcdf(x,b(3),b(4)));
% actual function
% b(1): lower lapse
% b(2): upper lapse
% b(3): peak
% b(4): variance

%% input
if isvector(Xdata)
    Xdata = Xdata(:);
    Ydata = Ydata(:);
end

% get some size information
[nDat,nFit] = size(Xdata);

% default guesses and bounds
dLB(:,1)    = 0     .*ones(nFit,1);
dLB(:,2)    = 0     .*ones(nFit,1);
dLB(:,3)    = -inf  .*ones(nFit,1);
dLB(:,4)    = 0     .*ones(nFit,1);

dUB(:,1)    = 1     .*ones(nFit,1);
dUB(:,2)    = 1     .*ones(nFit,1);
dUB(:,3)    = +inf  .*ones(nFit,1);
dUB(:,4)    = +inf  .*ones(nFit,1);

dIG         = initialguess_cumgauss(Xdata,Ydata);

% parse optional
Default = [
    {1,'silent',false}
    {2,'lowerbound',dLB}
    {2,'lb',dLB}
    {3,'upperbound',dUB}
    {3,'ub',dUB}
    {4,'initialguess',dIG}
    {4,'ig',dIG}
    {5,'lapse','symmetric'}
    ];
[
    flSilent,...
    LB,...
    UB,...
    IG,...
    lapseMethod,...
    ] = parse_optional_arg(Default,varargin{:});

% When IG is out of bounds add small value. Otherwise it breaks on
% "undefined values at initial point"
% or at
% "Input must be real and full"
% the latter needs addition/subtraction of > 1e-3
MatchUB     = IG >= UB;
MatchLB     = IG <= LB;
IG(MatchUB) = UB(MatchUB)-1e-3;
IG(MatchLB) = LB(MatchLB)+1e-3;

%% do non linear fit
switch lapseMethod
    case 'symmetric'
        cfun = @(b,x) cumgaussfun([b(1) b(1) b(3) b(4)],x);
    case 'asymmetric'
        cfun = @(b,x) cumgaussfun([b(1) b(2) b(3) b(4)],x);
    case 'none'
        cfun = @(b,x) cumgaussfun([0 0  b(3) b(4)],x);
end
[out,outvar,outfun,stat,outb] = anyfit(cfun,Xdata,Ydata,...
    'ig',IG,'lb',LB,'ub',UB,...
    'silent',flSilent,'plot',~flSilent);
switch lapseMethod
    case 'symmetric'
        out = out([1 1 3 4]);
        outvar = outvar(:,[1 1 3 4]);
    case 'none'
        out = [0 0 out([3 4])];
        outvar = [zeros(2,2) outvar(:,[3 4])];
end

%%
c=0;
if nargout > c
    c=c+1;
    varargout{c} = out;
end
if nargout > c
    c=c+1;
    varargout{c} = outvar;
end
if nargout > c
    c=c+1;
    varargout{c} = outfun;
end
if nargout > c
    c=c+1;
    varargout{c} = stat;
end
if nargout > c
    c=c+1;
    varargout{c} = outb;
end
