...
% V1 BNB March 2018: modifying the b reference values to work with the
% parameter order that comes from MWks surface fits.

function fullbw = vonmisesbandwidth_B(b)

% calculate half the amplitude of the vonmises at its peak
%halfheight = b(:,2) .* exp(b(:,4)) ./ (2.0*pi*besseli(0,b(:,4))) / 2 ;
halfheight = b(1) .* exp(b(4)) ./ (2.0*pi*besseli(0,b(4))) / 2 ;
...
errfun = @(b,h,x) abs(b(1).*exp(b(4)*cosd(x))/(2.0*pi*besseli(0,b(4))) - h);
