...
% version 1.1 - romesh.kumbhani@nyu.edu - 2013-01-10 - minor optimizations
%
%% defaults
            ...
            if (fontsize>0)
                if ~exist('Xticklabels','var')
                    if str2double(xticklabels(i,:)) == round(str2double(xticklabels(i,:)))
                        tempstring = sprintf('%.0f',str2double(xticklabels(i,:)));
                    else
                        tempstring = sprintf('%.1f',str2double(xticklabels(i,:)));
                    end
                else
                    if iscell(xticklabels)
                        tempstring = sprintf('%s',xticklabels{i});
                    else
                        tempstring = sprintf('%.0f',xticklabels(i));
                    end
                end
                text(xticklocations(i)*1.005,axislimits(3) - 0.10*axisranges(2),tempstring,...
                    'horizontalalignment','center','fontsize',fontsize,'fontname',fontname,'fontangle',fontangle,...
                    'fontweight',fontweight);
                
            end
            if isempty(tempstring)
                line([xticklocations(i) xticklocations(i)],[axislimits(3)-.05*axisranges(2) axislimits(3)-.06*axisranges(2)],...
                    'color','k','linewidth',xwidth);
            else
                line([xticklocations(i) xticklocations(i)],[axislimits(3)-.05*axisranges(2) axislimits(3)-.07*axisranges(2)],...
                    'color','k','linewidth',xwidth);
            end
        end
    end
            ...
            if (fontsize>0)
                if ~exist('Yticklabels','var')
                    if str2double(yticklabels(i,:)) == round(str2double(yticklabels(i,:)))
                        tempstring = sprintf('%.0f',str2double(yticklabels(i,:)));
                    else
                        tempstring = sprintf('%.1f',str2double(yticklabels(i,:)));
                    end
                else
                    if iscell(yticklabels)
                        tempstring = sprintf('%s',yticklabels{i});
                    else
                        tempstring = sprintf('%.0f',yticklabels(i));
                    end
                end
                text(axislimits(1) - 0.09*axisranges(1),yticklocations(i),tempstring,...
                    'horizontalalignment','right','fontsize',fontsize,'fontname',fontname,'fontangle',fontangle,...
                    'fontweight',fontweight);
            end
            if isempty(tempstring)
                line([axislimits(1)-.05*axisranges(1) axislimits(1)-.06*axisranges(1)],[yticklocations(i) yticklocations(i)],...
                    'color','k','linewidth',ywidth);
            else
                line([axislimits(1)-.05*axisranges(1) axislimits(1)-.07*axisranges(1)],[yticklocations(i) yticklocations(i)],...
                    'color','k','linewidth',ywidth);
            end
        end
    end
