...
%
function breakInfo = breakxaxis2(varargin)
    %Validate Arguements
    ...
    if strcmp(mainYTickLabelMode,'auto')
        ...
        xtickangle(45)
        set(leftAxes,'XTick',xTick(1:(end-1)), 'tickdir','out'); 
        xtickangle(45)
    end
    ...
    set(rightAxes,'Position', rightPosition, ...
        ...
        'Layer','top');
    if strcmp(mainLayer,'bottom')
        set(rightAxes,'Color','none','Layer','top');
         xtickangle(45)
    end
    ...
    if strcmp(mainYTickLabelMode,'auto')
        ...
        xtickangle(45)
        if length(xTick)<=4
            set(rightAxes,'XTick',round(xTick(end)), 'tickdir','out','Layer','top');
        else
            set(rightAxes,'XTick',[round(xTick(1)),round(xTick(end))] , 'tickdir','out','Layer','top');
        end  
    end
        %Create the Annotations layer, if the Layer is top, draw the axes on
    %top (e.g. after) drawing the left and right pane
    if strcmp(mainLayer,'top')
        annotationAxes = CreateAnnotaionAxes(mainAxes,mainParent);
        set(annotationAxes, 'Color','none', 'tickdir','out','Layer','top');
    end
    ...
    set(breakAxes,'Color','none', 'tickdir','out','Layer','top');
    ...
    set(breakAxes,'Position',breakPosition, 'tickdir','out','Layer','top');
    set(breakAxes,'Units',orignalUnits, 'tickdir','out','Layer','top');
    ...
    line('xData',xPoints1(1:3),'ydata',yPoints(1:3),'Parent',breakAxes,'Color',mainYColor,'LineWidth',mainLineWidth);
    line('xData',xPoints2(1:3),'ydata',yPoints(1:3),'Parent',breakAxes,'Color',mainYColor,'LineWidth',mainLineWidth);

    set(breakAxes,'Visible','off','tickdir','out','Layer','top');
