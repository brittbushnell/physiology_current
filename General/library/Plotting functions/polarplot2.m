function [varargout]=polarplot2(theta,rho,varargin)
...
%  default: "on"
%
% polarplot(...,'MaxRhoUnits','string')
%  sets the MaxRho unit that follows the value. Set to '' or [] to turn off
%  the label. This is string is case sensitive.
%  default: "ips"
%
    ...
    if ~mod(nvarargin,2) % number of arguments have to be in pairs!
        for i=1:2:nvarargin % for each pair
            switch lower(varargin{i}) % does the first pair equal one of the following:
                ...
                case 'showmaxrho' % matches ShowMaxRho
                    if strcmpi(varargin{i+1},'off') % if ShowMaxRho was "off"
                        ShowMaxRho = 0;
                    end
                case 'maxrhounits' % matches MaxRhoUnits
                    if ~isempty(varargin{i+1}) % if MaxRhoUnits isn't empty
                        if ischar(varargin{i+1})
                            MaxRhoUnits = varargin{i+1};
                        else
                            warning('ExpoMatlab:polarplot','A non-string was provided. Converting entry to string.\n');
                            if isfloat(varargin{i+1})
                            MaxRhoUnits = sprintf('%.1f',varargin{i+1});
                            elseif isinterger(varargin{i+1})
                                    MaxRhoUnits = sprintf('%.1f',varargin{i+1});
                            end
                        end
                    end                    
...
if (isHoldOFF) % hold is OFF!
    ...
    if ShowMaxRho
        ...
        h=text(textX,textY,[num2str(MaxRho) ' ' MaxRhoUnits],'FontName','Helvetica','FontSize',FontSize,'FontAngle','Oblique','HorizontalAlignment','center');
        ...
        text(textX,textY,[num2str(MaxRho) ' ' MaxRhoUnits],'FontName','Helvetica','FontSize',FontSize,'FontAngle','Oblique','HorizontalAlignment','center');
    end
    axis off;
    if exist('linecolor','var')
