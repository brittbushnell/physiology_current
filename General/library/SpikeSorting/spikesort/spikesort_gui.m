function spikesort_gui(operation)
...
if ~isempty(WaveformInfo.selected)
    ...
    if ~isempty(unsel)
        im2 = genSpikeMapColor(WaveformInfo.WaveformsAsShown(unsel,:), WaveformInfo.Unit(unsel),tmprez1,tmprez1,y1,y2,colors,max(1,round(log10(length(unsel)/10))))/2;
    else
