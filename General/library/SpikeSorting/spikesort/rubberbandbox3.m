...
currentKeyFcn = get(gcf,'keypressfcn'); %store the current keypress function -ACS 05Aug2013
hold on;
...
set(gcf,'keypressfcn',''); %clear the keypressfcn temporarily to block input from the keyboard while mouse button is down -ACS 05Aug2013
%k=waitforbuttonpress;
uiwait(gcf);
...
set(gcf,'keypressfcn',currentKeyFcn); %restore the original keypressfcn -ACS 05Aug2013
try %Added try/catch to improve error handling... -02Apr2013 Adam C. Snyder
    delete(lh);
catch ME
