function spikesort_gui_pca_rk(operation)
...
%                         spikesort_gui_pca_rk('changeHistory');
%                     elseif numel(get(Handles.history,'string'))==1
%                         spikesort_gui_pca_rk('clear_history');
%                     end;
                ...
                case num2cell([30,31]) %up or down arrows
                    ...
                    spikesort_gui_pca_rk('load');
                case 32 %spacebar
                    %Resample waveforms:
                    spikesort_gui_pca_rk('sample');
                case num2cell([48:57,127]) %the number keys (0-9) or the delete key
                    ...
                    spikesort_gui_pca_rk('moveSelected'); %move the selected spikes to the sort code indicated
...
set(i,'ButtonDownFcn','spikesort_gui_pca_rk box');
...
%tmp=bsxfun(@rdivide,double(WaveformInfo.WaveformsAsShown),max(abs(double(WaveformInfo.WaveformsAsShown)),[],2));
tmp=double(WaveformInfo.WaveformsAsShown(:,1:48));
[~,score,~,~,explained] = pca(tmp);
maxval = 1.1.*max(abs(reshape(score(p,1:3),1,[])));
% fullhist = hist4(score(p,1:3),...
%     {linspace(-maxval,maxval,50),...
%     linspace(-maxval,maxval,50),...
%     linspace(-maxval,maxval,50)});

figure(Handles.pcaFigure);
clf;
colormap(1-gray(256));
%set(gca,'Nextplot','replaceChildren','FontSize',10);

% subplot(2,2,1);
% imagesc(linspace(-maxval,maxval,50),linspace(-maxval,maxval,50),squeeze(sum(fullhist,3))');
% hold on;
% tmp=plot(score(sel,1),score(sel,2),'k.');
% set(tmp,'Marker','o','MarkerFaceColor','r','MarkerEdgeColor','r','MarkerSize',5);
% hold off;
% axis(maxval.*[-1 1 -1 1]);
...
subplot(2,2,1);
tmp=plot(score(setdiff(p,sel),1),score(setdiff(p,sel),2),'k.',score(sel,1),score(sel,2),'k.');
set(tmp(1),'Marker','o','MarkerFaceColor','none','MarkerEdgeColor','k','MarkerSize',3);
if size(tmp,1)>1
    set(tmp(2),'Marker','o','MarkerFaceColor','none','MarkerEdgeColor','r','MarkerSize',5);
end
axis(maxval.*[-1 1 -1 1]);
set(gca,'xticklabel',[],'yticklabel',[],'box','off');
xlabel('PCA 1');
...
subplot(2,2,2);
tmp=plot(score(setdiff(p,sel),2),score(setdiff(p,sel),3),'k.',score(sel,2),score(sel,3),'k.');
set(tmp(1),'Marker','o','MarkerFaceColor','none','MarkerEdgeColor','k','MarkerSize',3);
if size(tmp,1)>1
    set(tmp(2),'Marker','o','MarkerFaceColor','none','MarkerEdgeColor','r','MarkerSize',5);
end
...
subplot(2,2,3);
tmp=plot(score(setdiff(p,sel),1),score(setdiff(p,sel),3),'k.',score(sel,1),score(sel,3),'k.');
set(tmp(1),'Marker','o','MarkerFaceColor','none','MarkerEdgeColor','k','MarkerSize',3);
if size(tmp,1)>1
    set(tmp(2),'Marker','o','MarkerFaceColor','none','MarkerEdgeColor','r','MarkerSize',5);
end
...
subplot(2,2,4);
bar(1:length(explained),cumsum(explained),'k');
...
set(i,'ButtonDownFcn','spikesort_gui_pca_rk rasterLim');
