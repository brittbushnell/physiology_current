    ...
    if dataT.goodCh(ch) == 1
        ...
        for dt = 1:numDots
            for dx = 1:numDxs
                ...
                polarplot(dataT.prefOri2thetaNoise(end,dt,dx,ch),max10,'sk');
                text(dataT.prefOri2thetaNoise(end,dt,dx,ch),max10+2,sprintf('%.1f \\theta x2',(dataT.prefOri2thetaNoise(end,dt,dx,ch))),'FontWeight','bold','FontAngle','italic','FontSize',11)
                ...
                title(sprintf('2\\theta OSI: %.3f',dataT.OriSelectIndex2thetaNoise(end,dt,dx,ch)));
                ndx = ndx+1;
            end
        end
        ...
        for dt = 1:numDots
            for dx = 1:numDxs
                ...
                if dataT.prefOri2thetaNoiseSig(end,dt,dx,ch) == 1
                    polarplot(dataT.prefOri2thetaNoise(end,dt,dx,ch),max10,'sk');
                    text(dataT.prefOri2thetaNoise(end,dt,dx,ch),max10+3,sprintf('%.1f',(dataT.prefOri2thetaNoise(end,dt,dx,ch))),'FontWeight','bold','FontAngle','italic','FontSize',11)
                end
