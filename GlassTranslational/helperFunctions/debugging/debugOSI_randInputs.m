...
ttl = suptitle({'Distribtions of permuted OSI values using different random inputs';'blue: all ori resps = 20 red: only one ori = 40'});
ttl.Position = [0.5,-0.025,0];
%% noise near 0, flat ori tuning (all 20)
resp_rand = rand(size(oris_rand));
...
for nb = 1:2000
    for or = 1:4
        ...
        ori2 = 2*(oris_randRadial(randi(length(resp_rand),1)));
        expon = 1i*(ori2);
        exVar = exp(expon);
        respVect(or,:) = baseSub*exVar;
        denomVect(or,:) = (abs(baseSub));
...
numNeg = length(oris_rand)/4;
negNdx = randi(length(oris_rand),[1,numNeg]);
...
for nb = 1:2000
    for or = 1:4
        ...
        ori2 = 2*(oris_randRadial(randi(length(resp_randNeg),1)));
        expon = 1i*(ori2);
        exVar = exp(expon);
        respVect(or,:) = baseSub*exVar;
        denomVect(or,:) = (abs(baseSub));
...
resp_rand100 = randi(100,size(oris_rand));
...
for nb = 1:2000
    for or = 1:4
        ...
        ori2 = 2*(oris_randRadial(randi(length(resp_rand100),1)));
        expon = 1i*(ori2);
        exVar = exp(expon);
        respVect(or,:) = baseSub*exVar;
        denomVect(or,:) = (abs(baseSub));
...
numBig = length(oris_rand)/4;
bigNdx = randi(length(oris_rand),[1,numBig]);
...
for nb = 1:2000
    for or = 1:4
        ...
        ori2 = 2*(oris_randRadial(randi(length(resp_randBig),1)));
        expon = 1i*(ori2);
        exVar = exp(expon);
        respVect(or,:) = baseSub*exVar;
        denomVect(or,:) = (abs(baseSub));
