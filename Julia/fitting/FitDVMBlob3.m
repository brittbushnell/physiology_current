function [param, bhat, funbest] = FitDVMBlob3(x, R, nStarts, param0);
% VER. 3 %%%%%%%%
% 2 vonMises*asymmetric Gaussian blobs. Kappa is allowed to vary (adjusting dir tuning
% width) for each blob. Gaussian is same for each blob, but parameter A
% allows for an asymmetric Gaussian
%
% Inputs:
...
fnDVMBlob = @(sc1, sc2, mux, muy, kappa, sdy, A, kappa2) ...
    sc1*fnVMBlob(mux, muy, kappa, sdy, A) + sc2*fnVMBlob(mux+pi, muy, kappa2, sdy, A);
fnBounds = @(sc1, sc2, mux, muy, kappa, sdy, A, kappa2) double(...
    (mux < 0) | (mux > max(x)) | (muy < 0) | (muy > max(y)) | ...
    (kappa <= 0) | (kappa > 20) | (sdy <= 0) | (A < 0) | ...
    (kappa2 <= 0) | (kappa2 > 20)) * 1/eps;
...
% %     sc1    sc2    mux  muy  kappa  sdy   A    kappa2
% lb = [-inf   -inf   0    0    0      0     0    0   ];
% ub = [ inf    inf   24   13   20    inf    inf  20  ];
...
for iistart = 1:nStarts
    ...
    if iistart < nStarts/2 | isempty(param0)
        %         sc1    sc2    mux    muy  kappa  sdy  A    kappa2
        BETA0 = [ 0.05   0.05   maxx   maxy  2     1    1    2 ];
    else
        BETA0 = param0;
    end
