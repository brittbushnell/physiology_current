function bin_spike_bushnell11_contourInt(fileNAME,prefix_dir,OUTPUT_DIR,plotNEVTimestamp)
% bin spike counts from nev file
% NOTE: FIX BEFORE USING - DOES NOT RETURN ALL NECESSARY INFORMATION 1/25/19
% Inputs:
    ...
    if length(stim_display_update_events(ind).data) > 2
        ...
        if sum((cur_time < success_times) .* (success_times < (cur_time + T_SUCCESS))) ~= 0
            ...
            if length(filename) >= 1
                if strcmp(filename{end},stim_display_update_events(ind).data{2}.name) == 0 
                    new_stim = 1;
                end
            else
                new_stim = 1;
            ...
            if new_stim == 1
                ...
                filename = [filename; stim_display_update_events(ind).data{2}.name];
                n_stim = n_stim + 1;
            end
        end
    end
...
save([OUTPUT_DIR, save_name], 'stimOn', 'stimOff', 't_stim', 'bins', 'filename');
