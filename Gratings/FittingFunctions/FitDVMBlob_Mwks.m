function [param, bhat, funbest] = FitDVMBlob_Mwks(x, R, nStarts)
% Inputs:
...
x = x/180 * pi;
xx = repmat(x, [7 1]);                         %%%% Adjust these values for however densely sampled your stimulus space is!
...
% below vv
fnVMBlob = @(mux,muy,kappa,sdy) exp(kappa*cos(xx-mux)) .*exp((-(yy-muy).^2)/(2*sdy^2));

fnDVMBlob = @(sc1,sc2,mux,muy,kappa,sdy,muy2,kappa2,sdy2) sc1*fnVMBlob(mux,muy,kappa,sdy) + sc2*fnVMBlob(mux + pi,muy2,kappa2,sdy2);
...
BETA0 = [0.05, maxx, maxy, 2, 1, maxy, 2, 1];
%% Use this section when fitting two blobs - using 0:360
% % sc1 = amplitude first blob
