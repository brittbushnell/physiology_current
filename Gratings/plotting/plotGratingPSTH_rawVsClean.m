function plotGratingPSTH_rawVsClean (cleanData,filename)
%%
...
for ch = 1:96
    ...
    blankResp = mean(smoothdata(cleanData.bins((cleanData.spatial_frequency == 0), 1:35 ,ch),'gaussian',3))./0.01;
    stimResp = mean(smoothdata(cleanData.bins((cleanData.spatial_frequency> 0), 1:35 ,ch),'gaussian',3))./0.01;
    plot(1:35,blankResp,'b','LineWidth',0.5);
    plot(1:35,stimResp,'b','LineWidth',2);
    
    
    blankResp = mean(smoothdata(rawData.bins((rawData.spatial_frequency == 0), 1:35 ,ch),'gaussian',3))./0.01;
    stimResp = mean(smoothdata(rawData.bins((rawData.spatial_frequency > 0), 1:35 ,ch),'gaussian',3))./0.01;
