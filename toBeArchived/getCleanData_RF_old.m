...
% run a shuffle test to determine if there's activity
for iC = 1:nChan  
    % Julia method: do shuffle test of responses for random values versus
    % recorded values
%     randDist    = normrnd(1,1,[1 nStimShown]);
%     chanDist    = sum(data.bins(:,startBin:endBin,iC));

     blankDist   = sum((data.bins((data.rf == 1000),startBin:endBin,iC)));
     stimDist    = sum((data.bins((data.rf ~= 1000),startBin:endBin,iC)));

    p  = shuffleTest2Dist(blankDist, stimDist, 1000);
    if p < 0.05
...
trialResp       = squeeze(sum(data.bins((data.rf ~= 1000),startBin:endBin,:),2));
