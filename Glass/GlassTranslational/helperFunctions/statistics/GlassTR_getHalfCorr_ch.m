function [reliabilityMetricByCond,chSplitHalfCorrDist,chReliabilityIndex] = GlassTR_getHalfCorr_ch(dataT)
    ...
    for dt = 1:numDots
        for dx = 1:numDxs
            for co = 1:numCoh
                for or = 1:numOris
                    for nb = 1:1000
                        relTrials = squeeze(dataT.GlassTRZscore(or,co,dt,dx,ch,:));
                        [randA(:,nb),aInt] = datasample(relTrials,length(relTrials)/2,'Replace',false);
                        randB(:,nb) = relTrials(~ismember(1:length(relTrials),aInt));
                    end
                    corrBoot = diag(corr(randA,randB,'rows','pairwise'));
