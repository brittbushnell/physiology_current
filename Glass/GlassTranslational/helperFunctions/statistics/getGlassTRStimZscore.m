function [GlassEachStimZscore,GlassAllStimZscore] = getGlassTRStimZscore(dataT)
% This code will work for either Concentric/radial patterns or
% translational.

% (or,co,ndot,dx,ch,:)
[numOris,numDots,numDxs,numCoh] = getGlassTRParameters(dataT);

for ch = 1:96
    allSpikes = squeeze(dataT.AllStimTRSpikeCount(ch,:));
    GlassAllStimZscore(ch,:) = zscore(allSpikes,0,'all');
    chMu = nanmean(dataT.GlassTRSpikeCount(:,:,:,:,ch,:),'all');
    chStd = nanstd(squeeze(dataT.GlassTRSpikeCount(:,:,:,:,ch,:)),0,'all');
    for dt = 1:numDots
        for dx = 1:numDxs
            for co = 1:numCoh
                for or = 1:numOris
                    spikes = squeeze(dataT.GlassTRSpikeCount(or,co,dt,dx,ch,:));
                   % spikes(isnan(spikes)) = [];
                    GlassEachStimZscore(or,co,dt,dx,ch,:) = (spikes -  chMu)/chStd; %zscore(spikes,0,'all');
                end
            end
        end
    end
end



