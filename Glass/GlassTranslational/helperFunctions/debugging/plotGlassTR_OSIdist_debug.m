load WU_BE_GlassTR_V4_201708_may
...
for i = 1%:4
    ...
    leftOris = squeeze(data.LE.prefOri2thetaNoise(end,:,:,data.LE.goodCh == 1));
    leftSigs = squeeze(data.LE.OSI2thetaNoiseSig(end,:,:,data.LE.goodCh == 1));
...
for i = 1%:4
    ...
    SIL = deg2rad(leftOris(leftSigs == 1));
    SIL = reshape(SIL,[1,numel(SIL)]);
    SIL(isnan(SIL)) = [];
    cirMu = circ_mean(SIL(:));
    cirMu2 = cirMu+pi;
