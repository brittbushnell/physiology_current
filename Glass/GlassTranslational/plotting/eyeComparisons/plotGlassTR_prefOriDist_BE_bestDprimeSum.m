...
if location == 1
    figDir =  sprintf('~/bushnell-local/Dropbox/Figures/%s/%s/%s/EyeComps/prefOri/',REdata.animal, REdata.programID, REdata.array);
elseif location == 0
    figDir =  sprintf('~/Dropbox/Figures/%s/%s/%s/EyeComps/prefOri/',REdata.animal, REdata.programID, REdata.array);
end
if ~exist(figDir,'dir')
    mkdir(figDir)
end
...
dpRE = squeeze(REdata.linBlankDprime(:,end,:,:,:));
dpLE = squeeze(LEdata.linBlankDprime(:,end,:,:,:));
...
%%
pOris = squeeze(REdata.prefOri(end,:,:,:)); % get the preferred orientations for all 100% coherence stimuli
rSI = squeeze(REdata.OSI(end,:,:,:));
...
for ch = 1:96
    if REdata.goodCh(ch) == 1 && REdata.inStim(ch) == 1
...
% LE
pOris = squeeze(LEdata.prefOri(end,:,:,:)); % get the preferred orientations for all 100% coherence stimuli
lSI = squeeze(LEdata.OSI(end,:,:,:));
siA = [squeeze(vSumLE(1,1,:)),squeeze(vSumLE(1,2,:)),squeeze(vSumLE(2,1,:)),squeeze(vSumLE(2,2,:))];
[~,indL] = max(siA,[],2);

for ch = 1:96
    if LEdata.goodCh(ch) == 1 && LEdata.inStim(ch) == 1
        if indL(ch) == 1
...
suptitle({sprintf('%s %s %s distribution of preferred orientations square root transformed radius',LEdata.animal, LEdata.eye, LEdata.array);...
    'Best parameters based on dPrime Sum for each visually responsive channel'});

figName = [REdata.animal,'_',REdata.array,'_BE_prefOriDist_bestDprimeSum','.pdf'];
...
subplot(2,1,1)
hold on
histogram(indL(LEdata.goodCh==1 & LEdata.inStim==1,:),'normalization','probability','FaceColor','b','EdgeColor','w');
...
subplot(2,1,2)
hold on
histogram(indR(REdata.goodCh==1 & REdata.inStim == 1,:),'normalization','probability','FaceColor','r','EdgeColor','w');
set(gca,'tickdir','out','XTick',1:4,'XTickLabel',{'200 0.02','200 0.03','400 0.02','400 0.03'})
xlim([0 5])
ylim([0 .6])
xtickangle(45)
...
suptitle(sprintf('%s %s distribution of highest dPrimes across density and dx',REdata.animal, REdata.array))
figName = [REdata.animal,'_',REdata.array,'_BE_bestDprimeDistributions','.pdf'];
print(gcf, figName,'-dpdf','-fillpage')
%%
cd ../
folder = 'OSI';
...
text(xmax-0.1, 0.28, sprintf('num good channels: %d',sum(LEdata.goodCh & LEdata.inStim)));
...
title(sprintf('%s LE',LEdata.animal))
...
text(xmax-0.1, 0.28, sprintf('num good channels: %d',sum(REdata.goodCh & REdata.inStim)));
...
title(sprintf('%s RE',REdata.animal))

suptitle(sprintf('%s %s distribtion of OSIs for each ch greatest norm dPrime',REdata.animal, REdata.array));

figName = [REdata.animal,'_',REdata.array,'_BE_OSIdist_bestDprime','.pdf'];
print(gcf, figName,'-dpdf','-fillpage')
%%
pOrisR(pOrisR<0) = pOrisR(pOrisR<0)+180;
pOrisL(pOrisL<0) = pOrisL(pOrisL<0)+180;
REdata.prefOriBestDprime = pOrisR;
LEdata.prefOriBestDprime = pOrisL;
