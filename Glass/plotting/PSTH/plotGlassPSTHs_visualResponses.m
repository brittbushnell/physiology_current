...
for ch = 1:96
    ...
    if (tmp(1,ch) == 0)
        ...
        title(ch)
        set(gca,'Color','none','tickdir','out','XTickLabel',[],'YTick',[0,yMax],'FontAngle','italic');%,'FontSize',12);
        ylim([0 yMax])
        %ylim([0 inf])
    end
end

suptitle((sprintf('%s %s %s stim vs blank', REdata.animal, REdata.array)))

%% save figure
if contains(REdata.animal,'WV')
    if location == 1
        figDir =  sprintf('~/bushnell-local/Dropbox/Figures/%s/%/%s/8Deg/PSTH/',REdata.animal,REdata.programID,REdata.array);
    elseif location == 0
        figDir =  sprintf('~/Dropbox/Figures/%s/%/%s/8Deg/PSTH/',REdata.animal,REdata.programID,REdata.array);
    end
else
    if location == 1
...
for ch = 1:96
    ...
    set(gca,'Color','none','tickdir','out','XTickLabel',[],'FontAngle','italic','FontSize',7);
...
if contains(LEdata.animal,'WV')
    ...
    elseif location == 0
        figDir =  sprintf('~/Dropbox/Figures/%s/%/%s/8Deg/PSTH/',LEdata.animal,LEdata.programID,LEdata.array);
    end
else
    if location == 1
        figDir =  sprintf('~/bushnell-local/Dropbox/Figures/%s/%/%s/PSTH/',LEdata.animal,LEdata.programID,LEdata.array);
    elseif location == 0
        figDir =  sprintf('~/Dropbox/Figures/%s/%/%s/PSTH/',LEdata.animal,LEdata.programID,LEdata.array);
    end
end
cd(figDir)
...
for ch = 1:96
    ...
    %if REdata.goodCh(ch) == 1
    if contains(REdata.animal,'XT')
        blankResp = sum(smoothdata(REdata.bins((REdata.numDots == 0), 1:35 ,ch),'gaussian',3))./0.01;
        stimResp = sum(smoothdata(REdata.bins((REcohNdx), 1:35 ,ch),'gaussian',3))./0.01;
        plot(1:35,blankResp,'color',[0.14 0.63 0.42 0.7],'LineWidth',0.5);
        plot(1:35,stimResp,'color',[0.14 0.63 0.42 0.8],'LineWidth',2);
    else
        blankResp = sum(smoothdata(REdata.bins((REdata.numDots == 0), 1:35 ,ch),'gaussian',3))./0.01;
    ...
    title(ch)
    %set(gca,'Color','none','tickdir','out','XTickLabel',[],'FontAngle','italic','FontSize',7);
    set(gca,'Color','none','tickdir','out','FontAngle','italic','FontSize',6);
    ylim([0 inf])
end

suptitle((sprintf('%s %s RE stim vs blank', REdata.animal, REdata.array)))
...
title((sprintf('%s %s sum of each channels mean responses to stim vs blank, whole array', REdata.animal, REdata.array)))
ylabel('spikes/s')
...
title((sprintf('%s %s summed responses to stim vs blank, whole array', REdata.animal, REdata.array)))
ylabel('spikes/s')
xlabel('time 10 ms bins')
set(gca,'Color','none','tickdir','out','FontAngle','italic','FontSize',12);
legend('RE/AE','LE/FE')
