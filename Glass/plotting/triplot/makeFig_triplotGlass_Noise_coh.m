...
for i = 1:numCoh
    ...
    s = suptitle(sprintf('%s stimulus vs dipole d'' mean orientation %d%% coherence',V1data.conRadLE.animal,cohs(i)*100));
    ...
    %%
    figName = [V1data.conRadLE.animal,'_BE_bothArrays_triplot_meanOri_coh',num2str(cohs(i)*100)];
    set(gca,'color','none')
...
s.Position(4) = s.Position(4) + 0.02;
%%
figName = [V1data.conRadLE.animal,'_BE_bothArrays_triplot_coh_cmass',num2str(cohs(i)*100)];
set(gca,'color','none')
print(gcf, figName,'-dpdf','-bestfit')