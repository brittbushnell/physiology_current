...
[V1LEsort, V1REsort, V4LEsort, V4REsort] = getGlassSortedMats(V1data,V4data);
...
set(gcf,'Position',[pos(1), pos(2), 500, 400],'PaperSize',[7.5 3.5],'InvertHardcopy','off','color','w')
s = suptitle (sprintf('%s V1',V1data.trLE.animal));
    ...
    for ch = 1:96
        if V1trData.goodCh(ch) == 1 && V1trData.inStim(ch) == 1 && V1crData.inStim(ch) == 1 && V1crData.goodCh(ch) == 1
            ...
            if eye == 1
                ...
                scatter(v1LEconRadNdx(v1ndx),oriDiffV1(ch),30,'MarkerFaceColor',[0.2 0.4 1],'MarkerEdgeColor','w','LineWidth',0.5)
                plot([0 0], [0 90],':k')
                plot([0 90],[0 0],':k')
                ...
                scatter(v1REconRadNdx(v1ndx),oriDiffV1(ch),30,'MarkerFaceColor','r','MarkerEdgeColor','w','LineWidth',0.5)
                plot([0 0], [0 90],':k')
                plot([0 90],[0 0],':k')
...
figName = [V1data.trLE.animal,'_conRadNdxVStrOriDiff_V1'];
...
pos = get(gcf,'Position');
set(gcf,'Position',[pos(1), pos(2), 500, 400],'PaperSize',[7.5 3.5],'InvertHardcopy','off','color','w')
s = suptitle (sprintf('%s V4',V4data.trLE.animal));
s.Position(2) = s.Position(2)+0.02;
...
for eye = 1:2
    ...
    if contains(V4trData.animal,'XT')
    ...
    for ch = 1:96
        if V4trData.goodCh(ch) == 1 && V4trData.inStim(ch) == 1 && V4crData.inStim(ch) == 1 && V4crData.goodCh(ch) == 1
            ...
            if eye == 1
                ...
                scatter(v4LEconRadNdx(v4ndx),oriDiffV4(ch),30,'MarkerFaceColor',[0.2 0.4 1],'MarkerEdgeColor','w','LineWidth',0.5)
                plot([0 0], [0 90],':k')
                plot([0 90],[0 0],':k')
                ...
                scatter(v4REconRadNdx(v4ndx),oriDiffV4(ch),30,'MarkerFaceColor','r','MarkerEdgeColor','w','LineWidth',0.5)
                plot([0 0], [0 90],':k')
                plot([0 90],[0 0],':k')
...
figName = [V4data.trLE.animal,'_conRadNdxVStrOriDiff_V4'];
print(gcf, figName,'-dpdf','-bestfit')
%%
% figure%(4)
% clf
...
%
% suptitle(sprintf('%s distribution of local orientations relative to concentric',V4data.trLE.animal));
%
%
...
% t.Position(2) = t.Position(2)-0.05;
% %%
% figName = [V4data.trLE.animal,'_conRadNdxVStrOri_dist'];
% print(gcf, figName,'-dpdf','-bestfit')