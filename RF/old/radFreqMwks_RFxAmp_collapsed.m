...
for  r = 1:4
    subplot(2,2,r)
    if r == 1
        title(sprintf('%s radial frequency tuning baseline subtracted prefStims all ch', array));
        axis off
        set(gca,'Color','none')
    else
        hold on
        %        plot(xdata,REmuBS(r-1,:),'r-o','LineWidth',2)
        %         plot(2,REmuBS(end,1),'r-o','LineWidth',2)
        errorbar(xdata,REmuBS(r-1,:),REste(r-1,:),'r-o','LineWidth',2)
        errorbar(2,REmuBS(end,1),LEste(end,1),'r-o','LineWidth',2)
        plot([0 32], [32 0], ':k')
        
        
        %         plot(xdata,LEmuBS(r-1,:),'b-o','LineWidth',2)
        %         plot(2,LEmuBS(end,1),'b-o','LineWidth',2)
...
figName = ['WU_',array,'_allch','_RFxAmpBaseSub3'];
...
for  r = 1:4
    ...
    hold on
    if r == 1
        title(sprintf('%s Radial frequency tuning all ch pref params',array))
        axis off
        set(gca,'Color','none')
    else
        %         plot(xdata,REmu(r-1,:),'r-o','LineWidth',2)
        %         plot(2,REmu(end,1),'r-o','LineWidth',2)
        errorbar(xdata,REmu(r-1,:),REste(r-1,:),'r-o','LineWidth',2)
        errorbar(2,REmu(end,1),REste(end,1),'r-o','LineWidth',2)
        plot([1 128], [REstimResps(1,1) REstimResps(1,1)], '--r')
        
        
        %         plot(xdata,LEmu(r-1,:),'b-o','LineWidth',2)
        %         plot(2,LEmu(end,1),'b-o','LineWidth',2)
        errorbar(xdata,LEmu(r-1,:),LEste(r-1,:),'b-o','LineWidth',2)
        errorbar(2,LEmu(end,1),LEste(end,1),'b-o','LineWidth',2)
        plot([2 128], [LEstimResps(1,1) LEstimResps(1,1)], '--b')
...
figName = ['WU_',array,'_allCh','_RFxAmp3'];
saveas(gca,figName,'pdf')
%%
...
for r = 1:length(RFs)
    ...
    for a = 1:length(amps)
        ...
        if ~isempty(REndx)  % different RFs use different amplitudes, so this should skip any amplitudes that weren't run for that RF
            ...
            for i = 1:length(LEndx)
                REndxBS(i) = (REstimResps(19,REndx(i)));
                LEndxBS(i) = (LEstimResps(19,LEndx(i)));
...
figName = ['WU_',array,'_allch','_RFxAmpBaseSub_prefParam2'];
saveas(gca,figName,'pdf')
...
figName = ['WU_',array,'_allCh','_RFxAmp_prefParam2'];
saveas(gca,figName,'pdf')
