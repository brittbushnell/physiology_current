...
% newName = 'XT_radFreqLowSF_manLocs_V1_Dec2018_goodCh';
%% XT high SF
% file = 'XT_RE_radFreqHighSF_V1_Dec2018_cleaned';
% newName = 'XT_RE_radFreqHighSF_V1_Dec2018_goodCh';

file = 'XT_RE_radFreqHighSF_V4_Dec2018_cleaned';
newName = 'XT_RE_radFreqHighSF_V4_Dec2018_goodCh';
%%
...
radFreq_basicPSTH(cleanData)
radFreq_LocationResponses(cleanData, endBin, saveData, location, textName)
...
%badCh = [2 3 6 8 10 11 14 19 20 23 24 25 26 27 28 30 32 35 36 38 39 40 41 44 45 47 55 60 69 72 73 74 75 76 77 78 79 81 83 86 88 89 90 92 93 94 95 96];
%badCh = [2 14 19 26 27 39 55 76 83 92 94 95 96];

%07/06 data V4
% badCh = [2 3 5 7 8 9 10 11 12 13 14 18 19 20 23 24 25 26 27 28 29 30 32 33 36 38 39 40 41 44 45 47 55 60 67 68 64 69 70 71 72 73 74 76 77 78 79 81 83 85 88 89 90 91 92 93 94 95 96];
% badCh = [2 5 14 18 19 26 39 44 55 57 59 61 83 88 92 94 95 96];
%% XT
% V4 lowSF
% badCh = [2,10,11,12,14,16,18,20,23,24,27,61];
% badCh = [2,10,11,12,14,16,18,20,23,24,27,61];

% V4 highSF
% RE
badCh = [10 14 18 33 77];
...
% V1 highSF
% RE
% badCh = [1 2 3 4 6 8 10 11 14 33 34 35 39 61 65 68 69 70 71 72 73 74 75 76 80 94];
%LE
% badCh = [];
%%
...
if dbg == 1
    for ch = 1:numCh
        if cleanData.goodCh(ch) == 1
            maxX = max(cleanData.stimResps{ch}(end-3,:));
            maxY = max(cleanData.stimResps{ch}(end-2,:));
            
            figure
            hold on
            plot(cleanData.stimResps{ch}(end-3,:),cleanData.stimResps{ch}(end-2,:),'ko')
            plot([0 maxX], [0 maxY],'--')
