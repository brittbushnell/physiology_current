function [rfMuZ, rfStErZ,circMuZ, circStErZ,...
    rfMuSc, rfStErSc,circMuSc, circStErSc] = radFreq_getMuSerrSCandZ_WVXT(dataT,plotFlag)
...
for ch = 1:96
    ...
    if plotFlag == 1
        ...
        s = suptitle(sprintf('%s %s %s nanmean zscores as a funciton of amplitude ch %d',dataT.animal, dataT.array, eye,ch));
    ...
    for loc = 1:3
        for rad = 1:2
            ...
            if plotFlag == 1
                ...
                rf4a = squeeze(rfMuZ(1,1,:,rad,loc,ch));
                rf4b = squeeze(rfMuZ(1,2,:,rad,loc,ch));
                
                rf8a = squeeze(rfMuZ(2,1,:,rad,loc,ch));
                rf8b = squeeze(rfMuZ(2,2,:,rad,loc,ch));
                
                rf16a = squeeze(rfMuZ(3,1,:,rad,loc,ch));
                rf16b = squeeze(rfMuZ(3,2,:,rad,loc,ch));
                
                circ = squeeze(circMuZ(rad,loc,ch));
                ...
                title(sprintf('sf %d rad %d loc %d',rad,loc))
