function [rfMuZ, rfStErZ,circMuZ, circStErZ] = radFreq_getMuSEzscores(dataT)
...
for ch = 1:96
    ...
    s = suptitle(sprintf('%s %s %s mean zscores as a funciton of amplitude ch %d',dataT.animal, dataT.array, dataT.eye,ch));
    ...
    for loc = 1:3
        for sf = 1:2
            for sz = 1:2
                ...
                rfMuZ(1,1,:,sf,sz,loc,ch) = mean(rf4rot0);
                rfMuZ(1,2,:,sf,sz,loc,ch) = mean(rf4rot2);
                rfMuZ(2,1,:,sf,sz,loc,ch) = mean(rf8rot0);
                rfMuZ(2,2,:,sf,sz,loc,ch) = mean(rf8rot2);
                rfMuZ(3,1,:,sf,sz,loc,ch) = mean(rf16rot0);
                rfMuZ(3,2,:,sf,sz,loc,ch) = mean(rf16rot2);
                
                rfStErZ(1,1,:,sf,sz,loc,ch) = std(rf4rot0)/sqrt(length(rf4rot0));
                rfStErZ(1,2,:,sf,sz,loc,ch) = std(rf4rot0)/sqrt(length(rf4rot2));
                rfStErZ(2,1,:,sf,sz,loc,ch) = std(rf8rot0)/sqrt(length(rf8rot0));
                rfStErZ(2,2,:,sf,sz,loc,ch) = std(rf8rot0)/sqrt(length(rf8rot2));
                rfStErZ(3,1,:,sf,sz,loc,ch) = std(rf16rot0)/sqrt(length(rf16rot0));
                rfStErZ(3,2,:,sf,sz,loc,ch) = std(rf16rot0)/sqrt(length(rf16rot2));
                
                circMuZ(sf,sz,loc,ch) = mean(circZs);
                circStErZ(sf,sz,loc,ch) = std(circZs)/sqrt(length(circZs));
                %% plot curves
                subplot(3,4,ndx)
                
                rf4a = squeeze(rfMuZ(1,1,:,sf,sz,loc,ch));
                rf4b = squeeze(rfMuZ(1,2,:,sf,sz,loc,ch));
                
                rf8a = squeeze(rfMuZ(2,1,:,sf,sz,loc,ch));
                rf8b = squeeze(rfMuZ(2,2,:,sf,sz,loc,ch));
                
                rf16a = squeeze(rfMuZ(3,1,:,sf,sz,loc,ch));
                rf16b = squeeze(rfMuZ(3,2,:,sf,sz,loc,ch));
                
                circ = squeeze(circMuZ(sf,sz,loc,ch));
                ...
                title(sprintf('sf %d rad %d loc%d',sf,sz,loc))
