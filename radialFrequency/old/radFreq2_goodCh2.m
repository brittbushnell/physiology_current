...
%V4
% file = 'XT_radFreqLowSF_manLocs_V4_Dec2018_cleaned';
% newName = 'XT_radFreqLowSF_manLocs_V4_Dec2018_goodCh';
...
for ch = 1:96
    subplot(10,10,ch);
    hold on;
    plot((nanmean(squeeze(LEcleanData.bins((LEcleanData.radius==100) ,1:35,ch)),1)./.010),'k','LineWidth',2);
    if strfind(file,'nsp1')
        plot((nanmean(squeeze(LEcleanData.bins(((LEcleanData.radius~=100) & (LEcleanData.pos_x == min(unique(LEdata.pos_x)))),1:35,ch)),1)./.010),'c','LineWidth',2);
    else
        plot((nanmean(squeeze(LEcleanData.bins((LEcleanData.radius~=100),1:35,ch)),1)./.010),'c','LineWidth',2);
    end
...
for ch = 1:96
    subplot(10,10,ch);
    hold on;
    plot((nanmean(squeeze(REcleanData.bins((REcleanData.radius==100),1:35,ch)),1)./.010),'k','LineWidth',2);
    if strfind(file,'nsp1')
        plot((nanmean(squeeze(REcleanData.bins(((REcleanData.radius~=100) & (REcleanData.pos_x == min(unique(LEdata.pos_x)))),1:35,ch)),1)./.010),'r','LineWidth',2);
    else
        plot((nanmean(squeeze(REcleanData.bins((REcleanData.radius~=100),1:35,ch)),1)./.010),'r','LineWidth',2);
    end
    title(ch)
    set(gca,'Color','none','XColor','none')
    ylim([0 inf])
