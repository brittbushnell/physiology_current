...
file = 'WU_RadFreqLoc2_V4_20170707_002_new3_byStim';
newName = 'WU_RadFreqLoc2_V4_20170707_new3_RFxAmp';

location = 0;
startBin = 5;
endBin = 35;
date = '07/07';
dataType = 'raw, auto clean';
array = 'V4';
...
save(newName, 'LEdata');
fprintf('MAT file saved as: %s',newName)
...
for ndx = 1:4
    for  r = 1:3
        ...
        subplot(4,3,spot)
        topTitle = (sprintf('%s Mean responses for all SF x Size combos %s %s', array, date, dataType));
        set(gca,'Color','none')
        hold on
...
for ndx = 1:4
    ...
    for  r = 1:3
        subplot(1,3,r)
        topTitle = (sprintf('%s mean responses %s %s data', array, date, dataType));
        if r == 1
                    annotation('textbox',...
                [0.15 0.95 0.77 0.05],...
                'String',topTitle,...
...
for ndx = 3
    ...
    for  r = 1
        ...
        if ndx == 1
            title(sprintf('RF %d size = 1 deg sf = 1 %s %s data',RFs(r),date,dataType));
            if r == 3 %rf16
                set(gca,'tickdir','out','box','off','Color','none','XScale','log','XTick',[2 4 8 16 32 64 128],...
                    'XTickLabel',{'circle','22.5','45','90','180','360','720'},'XTickLabelRotation', 45,...
                    'FontName','Helvetica Neue','FontAngle','italic','FontSize',12)
        ...
        elseif ndx == 2
            title(sprintf('RF %d size = 1 deg sf = 2 %s %s data',RFs(r),date,dataType));
            if r == 3 %rf16
                set(gca,'tickdir','out','box','off','Color','none','XScale','log','XTick',[2 4 8 16 32 64 128],...
                    'XTickLabel',{'circle','22.5','45','90','180','360','720'},'XTickLabelRotation', 45,...
                    'FontName','Helvetica Neue','FontAngle','italic','FontSize',12)
        ...
        elseif ndx == 3
            title(sprintf('RF %d size = 2 deg sf = 1 %s %s data',RFs(r),date,dataType));
            if r == 3 %rf16
                set(gca,'tickdir','out','box','off','Color','none','XScale','log','XTick',[2 4 8 16 32 64 128],...
                    'XTickLabel',{'circle','45','90','180','360','720','1440'},'XTickLabelRotation', 45,...
                    'FontName','Helvetica Neue','FontAngle','italic','FontSize',12)
        ...
        elseif ndx == 4
            title(sprintf('RF %d size = 2 deg sf = 2 %s %s data',RFs(r),date,dataType));
            if r == 3 %rf16
                set(gca,'tickdir','out','box','off','Color','none','XScale','log','XTick',[2 4 8 16 32 64 128],...
                    'XTickLabel',{'circle','45','90','180','360','720','1440'},'XTickLabelRotation', 45,...
                    'FontName','Helvetica Neue','FontAngle','italic','FontSize',12)
...
for rf_n = 1:3
    ...
    if rf_n == 1
        topTitle = sprintf('Distribution of slopes across eyes for each RF set %s %s data',date,dataType);
        annotation('textbox',...
            [0.2 0.96 0.65 0.05],...
            'LineStyle','none',...
            'String',topTitle,...
...
topTitle = sprintf('Slopes across eyes for all parameters %s %s data',date,dataType);
annotation('textbox',...
    [0.2 0.96 0.65 0.05],...
    'LineStyle','none',...
    'String',topTitle,...
...
ylabel('% channels');
title(sprintf('RF ocular dominance %s, %s data',date,dataType));
