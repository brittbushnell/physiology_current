...
file = 'WU_RadFreqLoc2_V4_20170707_cleaned2x_goodCh';
array = 'V4';

location = 1; %0 = laptop 1 = desktop 2 = zemina
useMean = 1; % if set to 1, will use mean to compute data points in fig 1, if set to 0 will use median

%%
load(file);
%%
RFs   = unique(LEcleanData.rf);
amps  = unique(LEcleanData.amplitude);
phase = unique(LEcleanData.orientation);
SFs   = unique(LEcleanData.spatialFrequency);
xLocs = unique(LEcleanData.pos_x);
yLocs = unique(LEcleanData.pos_y);
rads  = unique(LEcleanData.radius);

numCh = size(LEcleanData.bins,3);
%REcleanData.goodCh = REcleanData.goodChannels;
%LEcleanData.goodCh = LEcleanData.goodChannels;
...
if strfind(file,'V1')
    ...
    if location == 0
        cd ~/Dropbox/Figures/WU_Arrays/RF/V4/0707
    elseif location == 1
        cd /users/bushnell/bushnell-local/Dropbox/Figures/wu_Arrays/RF/V4/0707
    elseif location == 2
        cd ~/Figures/V4/RadFreq/0707
    end
end
%%
        ...
        for ch = 1:numCh
            ...
            for r = 1:length(RFs)
                ...
                for a = 1:length(amps)
                    ...
                    REndx = find((REcleanData.stimResps{ch}(1,:) == RFs(r)) & ...
                        (REcleanData.stimResps{ch}(2,:) == amps(a)) & ...
                        (REcleanData.stimResps{ch}(5,:) == sz_rad) &...
                        (REcleanData.stimResps{ch}(4,:) == circ_sf));
                    
                    
                    LEndx = find((LEcleanData.stimResps{ch}(1,:) == RFs(r)) & ...
                        (LEcleanData.stimResps{ch}(2,:) == amps(a)) & ...
                        (LEcleanData.stimResps{ch}(5,:) == sz_rad) & ...
                        (LEcleanData.stimResps{ch}(4,:) == circ_sf));
...
%                             REndxBS(i) = (REcleanData.stimResps{ch}(19,REndx(i)));
%                             LEndxBS(i) = (LEcleanData.stimResps{ch}(19,LEndx(i)));
%                             
%                             REndx2(i) = (REcleanData.stimResps{ch}(18,REndx(i)));
%                             LEndx2(i) = (LEcleanData.stimResps{ch}(18,LEndx(i)));
%                             
%                             REste1(i) = REcleanData.stimResps{ch}(20,REndx(i));
%                             LEste1(i) = LEcleanData.stimResps{ch}(20,LEndx(i));
                            REndxBS(i) = (REcleanData.stimResps{ch}(end-2,REndx(i)));
                            LEndxBS(i) = (LEcleanData.stimResps{ch}(end-2,LEndx(i)));
                            
                            REndx2(i) = (REcleanData.stimResps{ch}(end-3,REndx(i)));
                            LEndx2(i) = (LEcleanData.stimResps{ch}(end-3,LEndx(i)));
                            
                            REste1(i) = REcleanData.stimResps{ch}(end-1,REndx(i));
                            LEste1(i) = LEcleanData.stimResps{ch}(end-1,LEndx(i));
                        end
                        LEmuTS(r,ndx) = nanmean(LEndxBS);
                        REmuTS(r,ndx) = nanmean(REndxBS);
                        
                        LEmuT(r,ndx) = nanmean(LEndx2);
                        REmuT(r,ndx) = nanmean(REndx2);
                        
                        REsteT(r,ndx) = 0.01;%nanmean(REste1);
                        LEsteT(r,ndx) = 0.01;%nanmean(LEste1);
            ...
            if useMean == 0
                % uses median
                LEmuBS{ch} = LEmuTS-LEcleanData.blankResps{ch}(end-2);
                REmuBS{ch} = REmuTS-REcleanData.blankResps{ch}(end-2);
            else
                % uses nanmean
                LEmuBS{ch} = LEmuT-LEcleanData.blankResps{ch}(end-2);
                REmuBS{ch} = REmuT-REcleanData.blankResps{ch}(end-2);
            end
        end
        all_RE(:,:,:,sz_sf_ndx) = cat(3,(REmuBS{logical(REcleanData.goodCh)}));
        all_LE(:,:,:,sz_sf_ndx) = cat(3,(LEmuBS{logical(LEcleanData.goodCh)}));
...
% for ch = 1:numCh
%     if (REcleanData.goodCh(ch) + LEcleanData.goodCh(ch)) ~=0
%
%         figure;
%         %clf;
%
...
%     end
%     %pause;
% %     if REcleanData.goodCh(ch) == 1 || LEcleanData.goodCh(ch) == 1
...
%average plots
xdata = [4,8,16,32,64,128];
mu_all_RE = squeeze(nanmean(all_RE,3));
mu_all_LE = squeeze(nanmean(all_LE,3));
...
for ndx = 1:4
    for  r = 1:3
        ...
        if spot == 1
            annotation('textbox',...
                [0.15 0.95 0.77 0.05],...
                'String',{'Mean responses for all SF x Size combinations 07/07'},...
                'FontWeight','bold',...
                'FontSize',16,...
                'FontAngle','italic',...
                'EdgeColor','none');
...
for rf_n = 1:3
    ...
    if rf_n == 1
        topTitle = 'Distribution of slopes across eyes for each RF set 07/07 clean data';
        annotation('textbox',...
            [0.2 0.96 0.65 0.05],...
            'LineStyle','none',...
            'String',topTitle,...
...
%%
all_LE_slopes_mu = nanmean(log10(abs(all_LE_slopes)),2);
all_RE_slopes_mu = nanmean(log10(abs(all_RE_slopes)),2);

figure

topTitle = 'Slopes across eyes for all parameters 07/07 clean data';
annotation('textbox',...
    [0.2 0.96 0.65 0.05],...
    'LineStyle','none',...
    'String',topTitle,...
...
subplot(2,2,4);
hold on;
plot(log10([4 8 16]),nanmean(squeeze(all_LE_slopes_mu)'),'b');
plot(log10([4 8 16]),nanmean(squeeze(all_RE_slopes_mu)'),'r');
set(gca,'tickdir','out');
ylabel('Slope (spikes/octave)');
xlabel('RF');
set(gca,'tickdir','out');
...
LE_c = zeros(1,96);
LE_c(logical(LEcleanData.goodCh)) = nanmean(all_LE_circle,2);
RE_c = zeros(1,96);
RE_c(logical(REcleanData.goodCh)) = nanmean(all_RE_circle,2);
