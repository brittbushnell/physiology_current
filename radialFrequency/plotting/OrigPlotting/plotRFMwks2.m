...
if strfind(file,'V1')
    if location == 0
        cd ~/Dropbox/Figures/WU_Arrays/RF/V1/Location
    elseif location == 1
    ...
    if location == 0
        cd ~/Dropbox/Figures/WU_Arrays/RF/V4/Location
    elseif location == 1
...
for ch = 1:numChannels
    ...
    axis square xy
    colorbar
    title(sprintf('%s ch %d AE baseline subtracted RadFreq responses', array, ch))
    xlabel('x coordinate (deg)')
    ylabel('y coordinate (deg)')
    
    h = colorbar;
    ylabel(h,'sp/s','FontSize',12)
    text(0.6, 0.6, sprintf('%.2f',REdata.blankResp(1,ch)),'Color','red','FontSize',12,'FontWeight','bold','FontAngle','italic','FontName','Helvetica Neue')
...
for ch = 1:numChannels
    ...
    axis square xy
    colorbar
    title(sprintf('%s ch %d FE baseline subtracted RadFreq responses', array, ch))
    xlabel('x coordinate (deg)')
    ylabel('y coordinate (deg)')
    
    h = colorbar;
    ylabel(h,'sp/s','FontSize',12)
    text(0.6, 0.6, sprintf('%.2f',LEdata.blankResp(1,ch)),'Color','red','FontSize',12,'FontWeight','bold','FontAngle','italic','FontName','Helvetica Neue')
