function plotRF_SigSizeBars(LEdata,REdata)
%%
...
LE4  = squeeze(LEdata.sigSize(1,:));       RE4  = squeeze(REdata.sigSize(1,:));
LE8  = squeeze(LEdata.sigSize(2,:));       RE8  = squeeze(REdata.sigSize(2,:));
LE16 = squeeze(LEdata.sigSize(3,:));       RE16 = squeeze(REdata.sigSize(3,:));

LEgCh = sum(LEdata.goodCh);
REgCh = sum(REdata.goodCh);

LERF4pref1 = sum(squeeze(LEdata.sizeCorrDiff(1,:)) > 0 & LE4 == 1);
LERF4pref2 = sum(squeeze(LEdata.sizeCorrDiff(1,:)) < 0 & LE4 == 1);

LERF8pref1 = sum(squeeze(LEdata.sizeCorrDiff(2,:)) > 0 & LE8 == 1);
LERF8pref2 = sum(squeeze(LEdata.sizeCorrDiff(2,:)) < 0 & LE8 == 1);

LERF16pref1 = sum(squeeze(LEdata.sizeCorrDiff(3,:)) > 0 & LE16 == 1);
LERF16pref2 = sum(squeeze(LEdata.sizeCorrDiff(3,:)) < 0 & LE16 == 1);

LEy = [LERF4pref1/LEgCh, LERF4pref2/LEgCh; LERF8pref1/LEgCh, LERF8pref2/LEgCh; LERF16pref1/LEgCh, LERF16pref2/LEgCh];
LEx = [0.05 4.15 8.3];

RERF4pref1 = sum(squeeze(REdata.sizeCorrDiff(1,:)) > 0 & RE4 == 1);
RERF4pref2 = sum(squeeze(REdata.sizeCorrDiff(1,:)) < 0 & RE4 == 1);

RERF8pref1 = sum(squeeze(REdata.sizeCorrDiff(2,:)) > 0 & RE8 == 1);
RERF8pref2 = sum(squeeze(REdata.sizeCorrDiff(2,:)) < 0 & RE8 == 1);

RERF16pref1 = sum(squeeze(REdata.sizeCorrDiff(3,:)) > 0 & RE16 == 1);
RERF16pref2 = sum(squeeze(REdata.sizeCorrDiff(3,:)) < 0 & RE16 == 1);
...
t = title(sprintf('%s %s Proportion of channels with significant Size preferences',REdata.animal, REdata.array), 'FontSize',14);
t.Position(2) = t.Position(2) +0.012;

l = legend('LE 1deg','LE 2deg', 'RE 1deg', 'RE 2deg');
l.Box = 'off'; l.NumColumns = 2;
l.FontSize = 10;

figName = [LEdata.animal,'_BE_',LEdata.array,'_sigSize','.pdf'];
